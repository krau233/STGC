...
def preprocess_cora(feature_matrix,num_time_steps):
    low, high = -1, 1
    # Initialize list for time-based graphs
    result = []
    result.append(feature_matrix)
    np.random.seed(0)
    # 可能的噪声值
    values = [0, 1, -1]
    # 每个值出现的概率
    ...
    for t in range(num_time_steps - 1):
        noise = torch.from_numpy(np.random.choice(values, p=probabilities, size=feature_matrix.shape)).to('cuda')
...
def preprocess_cora_norm(feature,num_time_steps):
    ...
    np.random.seed(0)
    for t in range(num_time_steps - 1):
        noise = torch.randn(feature.size()).to('cuda') * 0.01
        noise_features = result[-1] + noise
        # Add features to nodes
        result.append(noise_features.clone())
    return torch.stack(result).permute(1,0,2)
...
class Transd2Ind:
    ...
    def __init__(self, dpr_data, keep_ratio,name,num_time_steps):
